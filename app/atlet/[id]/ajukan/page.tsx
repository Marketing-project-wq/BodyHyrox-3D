...
import { cartAdd } from "@/app/brand/actions";
...
export default async function TambahKeranjangPage({
  ...
  return wrap(
    <>
      <h1 className="font-condensed text-3xl font-bold uppercase">{m.cart_add}</h1>
      ...
      <div className="mt-5 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3">
        ...
        <div className="text-right">
          ...
          <div className="tabnum text-sm text-white">{formatIDR(zone.basePrice)}</div>
        </div>
      </div>

      <form action={cartAdd} className="mt-6 flex flex-col gap-4">
        <input type="hidden" name="athlete_id" value={a.id} />
        <input type="hidden" name="athlete_zone_id" value={zone.athleteZoneId} />
        <input type="hidden" name="back" value={`/atlet/${a.id}`} />
        <div>
          <label className="text-xs text-white/50">
            {m.aj_note} <span className="text-white/30">({m.br_optional})</span>
          </label>
